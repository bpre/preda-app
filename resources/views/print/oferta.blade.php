...
<body>
    <div class="header">
        ...
        <div class="params">
            ...
            <table class="credit">
                ...
                @if(substr($offer->bank, 0, 5) !== 'Getin' && $offer->show_benefit == true)

                    @if($offer->benefit == NULL)
                    <tr>
                        <td style="font-weight: bold; vertical-align: top;">Orientacyjna korzyść:</td>
                        <td>

                            @if($offer->currency_index == 'EUR')

                            przy kredycie powiązanym z EUR korzyść na ogół stanowi równowartość 35-60% kwoty kredytu - w {{ $persona_a }} przypadku szacunkowo powinna mieścić się w przedziale: <strong>{{ number_format(round(intval( $offer->amount*0.35), -3), 0, ',', ' ') }} - {{ number_format(round(intval($offer->amount*0.6), -3), 0, ',', ' ') }} zł</strong>

                            @else

                            standardowo korzyść stanowi równowartość 80-120% kwoty kredytu<br>
                            w {{ $persona_a }} przypadku szacunkowo: <strong>{{ number_format(round(intval( $offer->amount*0.8), -3), 0, ',', ' ') }} - {{ number_format(round(intval($offer->amount*1.2), -3), 0, ',', ' ') }} zł</strong>

                            @endif

                            <div style="margin-top: 8px">
                                (zależne od czasu kredytowania, kursu wypłaty, historii spłat, wysokości oprocentowania, terminu spłaty - szczegółowo wyliczymy dysponując zaświadczeniem z banku)
                            </div>
                        </td>
                    </tr>
                    @else
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <strong>ok. {{ number_format(round(intval($offer->benefit), -3), 0, ',', ' ') }} zł</strong>
        ...
        <p>
            Poniżej przedstawiamy parametry przygotowanej dla {{ $persona_a }} oferty. Podane kwoty są kwotami brutto (zawierają VAT).
        </p>
    ...
    <table class="pricing">
        ...
        @if($offer->start_procent_limit !== 100)
        <tr>
    ...
    @if(substr($offer->bank, 0, 5) !== 'Getin')
    <p>
        <strong>
    ...
    <h3>
        Kiedy przysługuje premia za wygranie sprawy w wariancie „Bezpieczny start”?
    </h3>
    ...
    <h3>
        ...
        @if($offer->is_paid_off)


            <p>
                W przypadku kredytu, który został już w całości spłacony, korzyść stanowi kwotę, którą bank jest zobowiązany zapłacić na rzecz kredytobiorcy w związku z nieważnością umowy (lub zawarciem ugody).
            </p>
    ...
    <h3>
        ...
        <p>
            Jeśli {{ $ma_persona }} pytania do naszej oferty lub {{ $chcialby_persona }} nawiązać współpracę z naszą kancelarią - uprzejmie prosimy o kontakt telefoniczny lub mailowy:
        </p>
        <ul>
            <li style="margin-bottom: 8px; float: left">
                ...
                @else
                    <a href="tel:+48666580580">666 580 580</a>
                @endif
            </li>
            <li style="margin-left: 250px">
                ...
                @else
                    <a href="mailto:kancelaria@preda.info">kancelaria@preda.info</a>
                @endif
        ...
        @if($offer->is_initial_offer)
            <p>
                <small>
                    _<br>
                    Oferta została przygotowana na podstawie informacji przekazanych przez Klienta na etapie wstępnym. Ostateczną decyzję o przyjęciu sprawy do prowadzenia Kancelaria podejmuje dopiero po zapoznaniu się ze wszystkimi istotnymi dokumentami oraz po ustaleniu w rozmowie z Klientem wszystkich istotnych okoliczności sprawy.
                </small>
            </p>
        @endif
