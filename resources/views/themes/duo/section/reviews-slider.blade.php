@php
    $surfaceClass = $alternate ? 'bg-secondary-100' : 'bg-secondary-200/70';
    $reviewCardClasses = 'overflow-hidden rounded-[1.5rem] p-6 shadow-sm sm:p-8 lg:p-10 ' . $surfaceClass;
...
>
    ...
    >
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between lg:px-2">
            <div class="grid min-w-0 gap-y-2 text-primary-900 sm:w-fit sm:grid-cols-[max-content_max-content] sm:items-center sm:gap-x-6 sm:gap-y-1.5">
                <div class="w-[8.5rem] text-center text-[1.05rem] font-bold tracking-[0.08em] text-primary-900">DOSKONAŁA</div>

                <div class="text-sm leading-tight font-medium text-secondary-600 sm:border-l sm:border-secondary-200/70 sm:pl-5 [&_strong]:font-semibold [&_strong]:text-primary-900">
                ...
                <div class="flex w-[8.5rem] items-center justify-center gap-1 [&_svg]:h-6 [&_svg]:w-6" aria-hidden="true">
                    @for($i = 0; $i < 5; $i++)
                ...
                @if(filled($googleReviewsUrl))
                    <a href="{{ $googleReviewsUrl }}" target="_blank" rel="noopener noreferrer" class="inline-flex sm:border-l sm:border-secondary-200/70 sm:pl-5">
                        <img src="{{ asset('images/google-logo.svg') }}" alt="Google" width="255" height="80" class="h-8 w-auto" loading="lazy" />
                    </a>
                @else
                    <span class="inline-flex sm:border-l sm:border-secondary-200/70 sm:pl-5">
                        <img src="{{ asset('images/google-logo.svg') }}" alt="Google" width="255" height="80" class="h-8 w-auto" loading="lazy" />
                    </span>
                @endif
            </div>
        ...
        >
            ...
            <li
                class="w-full shrink-0 snap-start basis-full md:basis-[calc((100%-1rem)/2)] 2xl:basis-[calc((100%-2rem)/3)]"
            >
                <figure
                    class="{{ $reviewCardClasses }} h-full"
                    x-data="{
                        expanded: false,
                        expandable: false,
                        update() {
                ...
                >
                    <div class="flex items-center gap-4">
                        <div class="flex min-w-0 flex-1 items-center gap-4">
                            ...
                            <div class="min-w-0">
                                <div class="text-base font-semibold tracking-tight text-primary-900">{{ $displayName }}</div>
                                <div class="mt-0.5 block text-sm font-medium text-secondary-500">{{ ile_opinii($review->amount) }}</div>
                            </div>
                        </div>
                    ...
                    <div class="mt-4 flex items-center gap-1">
                        ...
                        <time datetime="{{ optional($review->date)->format('Y-m-d') }}" class="ml-2 text-sm text-secondary-500">
